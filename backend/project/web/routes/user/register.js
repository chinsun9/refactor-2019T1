...
router.post('/', (req, res) => {
	...
	var results2 = client.query('insert into luciddb.USER values (' + (count+1) + ', "' + userId + '", "' + userPw + '", "' + userName + '", "' + userEmail + '", ' + userEmailStatus + ', "' + userBirth + '", ' + userSex + ', ' + userArea + ')');
	var results3 = client.query('insert into luciddb.SERVICEEOGORDER values (' + (count+1) + ', 0, 1, 2, 3, 4, 5, 6, 7, 8)');
